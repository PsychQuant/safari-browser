...
| Probe the target window for a blocking dialog — every targeting command, before it runs (#126) | AX read of that one window's children, 0.25 s messaging timeout; whole-app scan only on the failure paths and in `dialog list` | same | Accessibility — absent is reported once, never treated as "no dialog" | already non-HID — read-only, no `AXPress` |
