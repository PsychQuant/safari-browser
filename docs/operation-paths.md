...
| Probe the target window for a blocking dialog — every command that resolves through the shared document resolver, before it runs (#126; the native-target commands `close` / `pdf` / `tab focus` / `upload` / `save-image` / ordinary `screenshot` / `tabs --window` are #133; `screenshot --full` and `--element` reach the probe later through JavaScript) | AX read of that one window's children, 0.25 s messaging timeout; whole-app scan only on the failure paths and in `dialog list` | same | Accessibility — absent is reported once, never treated as "no dialog"; a target-window subtree that could not be walked still reads as `none` (#135 / #138) | already non-HID — read-only, no `AXPress` |
