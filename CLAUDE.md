...
- **Screenshot 不 tab-switch**（#26）：其他 window-only commands 會 `performTabSwitchIfNeeded`，但 screenshot 刻意不切 tab — 因為 screenshot 是觀察性的，切 tab 破壞非干擾契約。URL 指向 **background tab** 時，screenshot **fail-closed 拒絕**（`.backgroundTabNotCapturable`，Exit 1），印出 actionable stderr 訊息（指示先手動切到該 tab：Safari 點該 tab 或 `safari-browser tab N --window M`，或改用 document-scoped 的 `snapshot --url` / `get source --url` 讀 DOM content 而非 visible pixels）—— 因為 window-level 像素截圖無法 render 非當前 tab。**此拒絕是刻意的非干擾設計**（不偷偷切 tab、也不截到「錯的當前 tab」誤導 caller）。⚠ 錯誤訊息只在 stderr：**不要 `2>/dev/null` 抑制** screenshot，否則 designed refusal 退化成裸 `Exit code 1` 無法診斷（#68 doc-reconcile：先前本行誤述為「截當前 tab」，與實作 drift）。
...
- **`js` 的位置式目標在命令邊界 anchor**（#180）：`resolveToConcreteTarget` 只把 `.urlMatch` / `.documentIndex` 收斂成 `.resolvedTab`，而 `resolveScriptTarget` 只對 `.resolvedTab` 走捷徑——所以 `--window N --tab-in-window M` / `--window N` / 無 flag 的 `js` 過去**每一步**都重跑 resolver（`.windowTab` 每步一次完整列舉，六步六次，109 個分頁要 20 秒）。`JSCommand` 現在改走 `resolveToAnchoredTarget`：`.windowTab` 一次列舉、`.frontWindow` / `.windowIndex` 一個 `readWindowAnchor` round-trip（window id + current tab index），拿不到 anchor（0-tab window、無視窗）就原樣退回位置式目標，#87 / #97 的錯誤路徑逐字不變；anchor 成功後分頁在指令中途消失，改報 `anchoredTabGone`（`JSCommand.anchoredFailure`），**不**走 `documentNotFound`——後者會列出所有 profile 的全部分頁，而無 flag 的預設目標在 anchor 之前從不列（verify R1）。`--profile` + 視窗層級目標改以 resolved window id 讀 anchor，避免兩次 round-trip 之間 z-order index 指到別的 profile 的視窗。迴歸測試 `JSCommandRoundTripTests` 用假 Safari（`DaemonRequestContext.appleScriptRunner`）模擬 5 視窗 109 分頁，直接數列舉次數。**`resolveToConcreteTarget` 的契約刻意不動**——`TabCommand` / `OpenCommand` / `resolveProfileScoped` 在 `--profile` 路徑 pattern-match 它回的 `.windowIndex` / `.windowTab`，改了會靜默丟 window。同一 issue 把 `listAllWindowsScript` 改成每視窗兩個 Apple event（`URL of every tab` / `name of every tab`），per-tab 讀只留在 `on error` fallback；結構由 `WindowEnumerationBatchingTests` 與 `ZeroTabWindowGuardTests` 一起 pin。語意微調：無 flag 的 `js` 在命令開始時就釘住 front window 當下的 current tab，不再每步重讀。
