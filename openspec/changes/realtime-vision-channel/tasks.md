...
- [ ] 2.1 建立 `channel/` 目錄：`package.json`（依賴 `@modelcontextprotocol/sdk`）、`bun install`
- [ ] 2.2 實作 `channel/channel.ts`：MCP server with channel capability，宣告 `claude/channel` experimental capability，透過 StdioServerTransport 連接
- [ ] 2.3 實作 Channel instructions：撰寫 instructions 字串說明 `<channel source="safari-vision">` 事件格式

## 3. Monitor Loop：智慧 diff — 截圖 + VLM（Periodic screenshot and analysis、Change detection、Temporary file cleanup）

- [ ] 3.1 實作 Periodic screenshot and analysis：setInterval 呼叫 `safari-browser screenshot` + `safari-vision analyze`，預設 1500ms 間隔
- [ ] 3.2 實作 Change detection：比較前後兩次 VLM 輸出，只在不同時 Push page change notifications
- [ ] 3.3 實作 Temporary file cleanup：分析完成後刪除截圖暫存檔
- [ ] 3.4 支援 `SB_CHANNEL_INTERVAL` 環境變數覆寫間隔

## 4. Reply Tool：雙向通訊（Execute safari-browser commands via reply tool、Return command output、Command validation）

- [ ] 4.1 實作 `safari_action` MCP tool：ListToolsRequestSchema 註冊 tool schema（command + args）
- [ ] 4.2 實作 CallToolRequestSchema handler：Execute safari-browser commands via reply tool，用 execFileSync 呼叫 safari-browser CLI
- [ ] 4.3 實作 Command validation：白名單驗證 command 是合法的 safari-browser 子指令
- [ ] 4.4 實作 Return command output：回傳 stdout 作為 tool result

## 5. Plugin 結構整合與啟動流程

- [ ] 5.1 更新 `psychquant-claude-plugins/plugins/safari-browser/plugin.json`：加入 channel component 指向 `channel/channel.ts`
- [ ] 5.2 更新 SKILL.md：加入 channel 使用說明和啟動方式
- [ ] 5.3 在 `.mcp.json` 或 plugin manifest 中註冊 channel server
