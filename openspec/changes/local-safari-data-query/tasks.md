...
- [x] 1.3 讓「權限不足」與「檔案不存在」成為兩個可分辨的結果：`Errors.swift` 新增 `fullDiskAccessRequired` 與 `safariDataFileNotFound`，前者以非 0 結束、後者以 0 結束且 stdout 無資料列（滿足 `Full Disk Access failure is distinguished from missing data files`）。驗證：`ErrorsTests` 新增斷言涵蓋兩者的 exit code 與訊息通道，`swift test --filter ErrorsTests` 綠。
