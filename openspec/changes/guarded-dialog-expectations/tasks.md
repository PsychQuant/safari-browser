## 1. Guarded dismissal
- [x] 1.1 旗標與 raw message 的 TDD／validation。
- [x] 1.2 在實際 press snapshot 檢查 window ID，證明失配零次 press，保留既有 guards。
- [x] 1.3 harness 傳遞期望，交接替換測試、文件與回歸。
- [x] 1.4 凍結審查及 GUI 驗收，更新兩個 issue 與 PR。

- [x] 1.5 將同樣身份期望與不重試保護套用 legacy e2e-dialog.sh，接受實測 JavaScript heading 並傳精確原文；重新實機驗收。
