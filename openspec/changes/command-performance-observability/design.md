...
5. `daemon __serve`／`mcp` 常駐 host 不建立跨請求 collector；子 CLI worker 各自建立。daemon 的 `applescript.execute`／`exec.runScript` 接受 optional literal-Boolean `timing`，只有 true 啟用個別 handler collector；成功 RPC 的 result 可附 `timing` 摘要；handler 拋出時仍 finish error，經 request-local sink 存入該次 DaemonRequestContext，再於原 error envelope 旁附 top-level `timing`。不包裝原錯誤，不改分類或重試。舊 server 沒有 metadata 視為 unavailable，不為取得計時而重送。
...
7. benchmark 為 `scripts/benchmark-performance.py`，只提供固定唯讀／零等待情境：startup help、wait 0、exec wait batch、自己的 daemon namespace 與 MCP wait worker；GUI get-title／get-url 另需 `--live`，只建立並清理自己的 localhost fixture。不得接受任意修改命令重複執行。沒開 GUI 模式或環境不足列 SKIP；禁止 Print。warm-daemon samples 顯式要求 daemon 路徑並檢查 host 存活／fallback；即使 stdout 正確也拒絕退回直接執行或截斷診斷，停止後續 warm samples，不重建服務。
