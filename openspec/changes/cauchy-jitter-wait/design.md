...
**上述單調性只在數學上成立**（verify R1）：`--scale` 相對區間過大時，兩端 CDF 皆捨入成 0.5，`quantile` 把捨入雜訊乘上 `--scale`，計算出的截斷後中位數在 [a, b] 內也不再單調，甚至 F(a) = F(b)。實測 scale/(max−min) ≤ 100 時，於各種區間寬度（1e-3 至 1e7 ms）與偏移（至 1e12 ms）皆嚴格單調；1000 即失效。故 `--scale` 上限為 100 × (max − min)，超過者為驗證錯誤；另外驗證兩端中位數嚴格遞增、CDF 區間非空，並以回代確認解出的 μ 確實給出要求的中位數。在比值 100 時截斷分布在 [a, b] 上已近乎均勻，上限之外沒有實際用途。
