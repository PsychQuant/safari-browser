...
- **#20: Probe System Events before sending keystrokes** — `upload --native` and `navigateFileDialog` now run a 2-second `probeSystemEvents()` (wrapped inside `runShell(timeout:)` from #19) before touching the keyboard. If the probe fails, the bridge attempts a best-effort `restartSystemEvents()` (`killall "System Events"` + 1s sleep + re-probe, relying on launchd to relaunch) and retries once. A final failure surfaces as `SafariBrowserError.systemEventsNotResponding(underlying:)`, whose description includes an actionable recovery command the user can paste into a terminal. Previously the command would hang silently inside the combined osascript until the #19 watchdog fired — useful as a last resort but opaque.
